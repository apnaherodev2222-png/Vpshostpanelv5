...
Flow:
  ...
  4. It DMs the uploader directly with the result.
...
table — whichever decides first wins, and hosting_panel.py's run-gate
just checks `status == 'approved'` regardless of who set it.
...
if not APPROVAL_BOT_TOKEN:
    raise SystemExit("APPROVAL_BOT_TOKEN missing from .env — approval_bot needs its OWN bot token, "
                      "separate from the main hosting bot's token.")
...
def _conn():
    ...
    conn.execute("PRAGMA busy_timeout = 5000")  # main bot writes to the same file
    return conn
...
async def notify(user_id: int, text: str):
    try:
        ...
        logger.error(f"Could not notify {user_id}: {e}")
            ...
            for user_id, file_name in fetch_pending():
                ...
                if verdict == 'clear':
                    if decide(user_id, file_name, 'approved', 'no suspicious patterns found'):
                        await notify(
                            user_id,
                            f"✅ <code>{file_name}</code> passed automatic review — you can run it now."
                        )
                    ...
                    if decide(user_id, file_name, 'rejected', reason):
                        await notify(
                            ...
                            f"Automatic review flagged this upload as potentially illegal/abusive "
                            f"(matched: {reason}).\n\n"
                            f"⏱️ You're muted from running free-tier scripts for the next 2 hours. "
                            f"If this looks wrong, ask an admin to review it manually."
                        )
